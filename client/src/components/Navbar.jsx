      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <span className="text-xl font-bold text-white tracking-tight">StudySync</span>
          </Link>
            ...
            <div className="hidden lg:flex items-center gap-1">
              {filteredLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-all duration-200"
                >
                  {getIcon(link.icon)}
                  {link.label}
                </Link>
          ...
          <div className="flex items-center gap-4">
            {isAuthenticated ? (
              <>
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-3 p-1.5 pr-3 bg-slate-800/50 hover:bg-slate-800 rounded-full transition-colors border border-slate-700"
                  >
                  ...
                  {/* Dropdown Menu */}
                  {profileDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-slate-800 rounded-xl shadow-xl border border-slate-700 py-2 animate-in fade-in zoom-in duration-200">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-slate-700/50 transition-colors"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link to="/signup" className="px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg hover:from-indigo-600 hover:to-purple-700 transition-all shadow-lg shadow-indigo-500/25">
                  Get Started
                </Link>
              </div>
            )}
        ...
        {isAuthenticated && mobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-slate-800 space-y-1 animate-in slide-in-from-top-2 duration-200">
            {filteredLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
